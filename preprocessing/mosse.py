...
class MOSSE:
    def __init__(self, frame, rect, name):
        x1, y1, x2, y2 = rect
	self.name = name
	self.reject = False
        ...
        self.update(frame)
    def getImg(self):
	x , y = self.last_img.shape[0],self.last_img.shape[1]
	return self.last_img[0.1*x:0.9*x,0.1*y:0.9*y]
	...
	#h=h*2
        ...
        draw_str(vis, (x1, y2+16), self.name)
        return (x1, y1,x2, y2)
